...
from src.mapping.guesty_mapper import map_guesty_reservations
from src.mapping.hubspot_payload import to_hubspot_contact_payload
from src.sync.logger import log_event


DATA_PATH = Path(__file__).resolve().parents[2] / "data" / "guesty_sample_reservations.json"


def run_guesty_sync() -> dict:
    raw_records = json.loads(DATA_PATH.read_text())
    log_event("fetched_records", pms_platform="Guesty", count=len(raw_records))

    mapped_records = map_guesty_reservations(raw_records)
    clean_records, skipped_records = deduplicate_by_email(mapped_records)
    log_event("invalid_or_skipped_records", pms_platform="Guesty", count=len(skipped_records))
    ...
    log_event("duplicates_removed", pms_platform="Guesty", count=len(mapped_records) - len(clean_records) - len(skipped_records))

    payloads = [payload for record in clean_records if (payload := to_hubspot_contact_payload(record)) is not None]
    log_event("payloads_prepared", pms_platform="Guesty", count=len(payloads))

    result = safe_batch_upsert(FakeHubSpotClient(), payloads, batch_size=4)
    log_event("upsert_summary", pms_platform="Guesty", upserted=len(result["successful"]), failed=len(result["failed"]))
